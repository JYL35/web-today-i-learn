...
> crew_id(PK), nickname으로 이루어진 crew 테이블을 만든다.
...
ON crew.crew_id = attendance.attendance_id
WHERE attendance.attendance_date = '2025-03-04'
AND crew.nickname LIKE '디%';
```
...
INSERT INTO attendance
    (crew_id, attendance_date, start_time, end_time)
VALUES (
...
```sql
SELECT attendance_id, attendance.crew_id, nickname, attendance_date, start_time, end_time
FROM attendance
INNER JOIN crew
ON attendance.crew_id = crew.crew_id;
```
...
```sql
SELECT a.crew_id, c.nickname, COUNT(a.attendance_date) AS attendance_date_count
FROM attendance a
INNER JOIN crew c
ON a.crew_id = c.crew_id
...
```sql
SELECT a.attendance_date, COUNT(*) AS attendance_crew_count
FROM attendance a
INNER JOIN crew c
ON a.crew_id = c.crew_id
...
```sql
SELECT a.crew_id, c.nickname, MIN(a.start_time) AS earlyest_start_time, MAX(a.start_time) AS latest_start_time
FROM attendance a
INNER JOIN crew c
ON a.crew_id = c.crew_id
GROUP BY c.crew_id, c.nickname;
